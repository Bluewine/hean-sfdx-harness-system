...
- **`both` does faithfully catch a metaData component `pre` declares that `post` is missing.** Both phases convert into the same `./build/source-mdapi-metadata`, wiped once before the run starts and never between phases — the same shared, uncleared folder the real pipeline uses. Confirmed against a real org: a component nested in a shared container file (a `CustomField` inside an `.object` file) that `pre` declares and `post` doesn't fails `post`'s metadata deploy here exactly as it does in a real run, with the same "Not in package.xml" error. A standalone-file component left the same way (an `ApexClass`, a `Flow`, a `FlowDefinition`) is silently dropped instead — confirmed harmless, not a gap this tool hides.
