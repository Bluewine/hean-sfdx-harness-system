...
description: "When writing docs/pr-body/ output files in the create-pr skill, use Bash heredoc to override directly — never use the Write tool which requires reading first"
metadata: 
  node_type: memory
  type: feedback
...
When writing the rendered PR body to `docs/pr-body/{WORK-ID}.md`, use a Bash heredoc (`cat > file <<'EOF'`) to write directly and override any existing file without needing to read it first.
