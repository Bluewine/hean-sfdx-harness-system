...
The user discontinued the `manifest/last-deployed.xml` deployment audit trail — do not write, reference, or recreate it. `hean-harness:sfdx-deployer` rebuilds `.claude/manifest/{WORK-ID}.xml` with branch-manifest before every deploy, so components added after the file was first written are deployed too.
