...
`^@[A-Z]+-[0-9]+(-[A-Z]{2})?:\s[A-Z](.*)$`. It can only read a message given on the command line
with `-m`; a commit that opens an editor passes through unchecked, so the shape is still yours to
get right.
...
`[Sonar]` tag after the prefix, a missing prefix entirely. Skills that read history tolerate those
subjects on purpose. That tolerance is for reading what already exists; it is not permission to
